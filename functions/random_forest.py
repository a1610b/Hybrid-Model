...
def calc_accr_rf(stock_list, d):
    ...
    target_list = ['industry_return_1d', 'industry_return_5d', 'industry_return_20d', 'index_return_1d',
    ...
    for stock in stock_list:
        try:
            ...
            for target in target_list:
                ...
                classifier = RandomForestClassifier(min_samples_leaf=100, n_estimators=200,
                ...
                if 'return_1d' in target:
                    ...
                    temp_dict[target+'_enhanced_return_compound_ls'] = np.prod(stor_prob_y
                                                                               * test_data_set['return_1d'] + 1)
            ...
            for target in target_list_abs:
                ...
                classifier = RandomForestClassifier(min_samples_leaf=200, n_estimators=500,
                                                    random_state=0, n_jobs=-1, class_weight='balanced_subsample')
                classifier.fit(train_x, train_y)
                y_pre = classifier.predict(test_x)
                temp_dict[target] = round(np.nanmean(y_pre == test_y), 4)
...
def calc_accr():
    """
    Create random forest for all stock and calculate its performance.
    ...
    for i in range(24):
        ...
        os.remove('D:\\output\\rf_result' + str(i) + '.csv')
    final_result.to_csv('D:\\output\\rf_result.csv')
    print(final_result.mean())
    print(final_result.groupby('industry').mean())
    print(final_result.groupby('index').mean())
